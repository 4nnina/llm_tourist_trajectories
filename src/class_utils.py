...
class Config:
    ...
    def __init__(self, model_name, anchor_strategy, prompt_context):
        # Model configuration
        self.MODEL_NAME = model_name #"deepseek-coder:33b" #llama3.1:8b - deepseek-coder:33b
        self.TOP_K = 5  # Number of POI predictions
        
        # HPC optimization parameters - OPTIMIZED FOR HIGH THROUGHPUT
        self.MAX_CONCURRENT_REQUESTS = 12  # 4 GPUs × 3 requests per GPU (INCREASED)
        self.REQUEST_TIMEOUT = 300  # Reduced for better throughput (was 600)
        self.BATCH_SAVE_INTERVAL = 500  # Save results every N cards
        self.HEALTH_CHECK_INTERVAL = 600  # Check host health every N seconds
        
        # Retry and failure handling
        self.MAX_RETRIES_PER_REQUEST = 10
        ...
        self.RETRY_ON_503_WAIT = 60  # ✅ NUOVO: attesa specifica per 503
        self.MAX_503_RETRIES = 20    # ✅ NUOVO: retry dedicati per 503
        
        # Anchor rule for POI selection
        self.DEFAULT_ANCHOR_RULE = anchor_strategy#"middle"
        
        # Parallelism - OPTIMIZED FOR A100 64GB CAPACITY
        self.ENABLE_ROUND_ROBIN = True  # Abilita round-robin
        self.HOST_SELECTION_STRATEGY = "balanced"  # "round_robin", "performance", "balanced"
        self.MAX_CONCURRENT_PER_GPU = 3  # INCREASED: 3 richieste simultanee per A100 64GB

        self.CONTEXT = prompt_context
        
        # File paths
        self.OLLAMA_PORT_FILE = "ollama_ports.txt"
        self.LOG_DIR = "../logs"
        self.RESULTS_DIR =f"../results/{self.DEFAULT_ANCHOR_RULE}/{self.MODEL_NAME}/{self.CONTEXT}"
        self.DATA_DIR = "../data/verona"
        self.POI_FILE = f"{self.DATA_DIR}/vc_site.csv"

    def setup_logging(self) -> logging.Logger:
        """Configure logging with file and console output"""
        self.LOG_DIR.mkdir(exist_ok=True)
        log_file = self.LOG_DIR / f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ...
        logger_local.handlers.clear()  # Remove any existing handlers
...
class ConfigBaseline():
    ...
    def __init__(self, context, anchor_strategy):
        self.MODEL_NAME = 'baseline_heuristics'
        self.CONTEXT = context
        
        self.TOP_K = 5  # Number of POI predictions
        
        # Optimization parameters
        self.MAX_CONCURRENT_WORKERS = max(1, os.cpu_count() // 2) # Usa metà delle CPU
        self.BATCH_SAVE_INTERVAL = 1000  # Save results every N cards
        
        # Anchor rule for POI selection
        self.DEFAULT_ANCHOR_RULE = anchor_strategy
...
class OllamaConnectionManager:
    ...
    def __init__(self, conf_obj: Config):
        ...
        self.circuit_breaker = CircuitBreaker(
            failure_threshold=conf_obj.CIRCUIT_BREAKER_THRESHOLD,
            timeout=300
        )
    ...
    def setup_connections(self) -> List[str]:
        ...
        try:
            with open(self.conf_obj.OLLAMA_PORT_FILE, "r") as f:
            ...
            if "," in ports_str:
                ...
                self.rate_limiter = Semaphore(len(self.hosts) * self.conf_obj.MAX_CONCURRENT_PER_GPU)  # 2 richieste per GPU contemporaneamente
            ...
            raise RuntimeError(f"Configuration file {self.conf_obj.OLLAMA_PORT_FILE} not found")
        except Exception as e:
    ...
    def test_model_availability(self, model: str = None) -> bool:
        """Test if the specified model is available on all hosts"""
        working_hosts = 0
        model = model or self.conf_obj.MODEL_NAME
    ...
    def get_chat_completion(self, prompt: str, model: str = None) -> Optional[str]:
    ...
    def _make_request_with_retry(self, prompt: str, model: str) -> Optional[str]:
        ...
        for attempt in range(1, self.conf_obj.MAX_RETRIES_PER_REQUEST + 1):
            ...
            try:
                ...
                resp = requests.post(
                    f"{host}/api/chat",
                    json=payload,
                    timeout=self.conf_obj.REQUEST_TIMEOUT,
                ...
                if resp.status_code == 503:
                    ...
                    if service_unavailable_count <= self.conf_obj.MAX_503_RETRIES:
                        wait_time = self.conf_obj.RETRY_ON_503_WAIT * min(service_unavailable_count, 5)
            ...
            if attempt < self.conf_obj.MAX_RETRIES_PER_REQUEST:
                if service_unavailable_count > 0:
                    # Shorter backoff for 503 errors since we already waited above
                    backoff_time = min(5, self.conf_obj.BACKOFF_BASE ** (attempt - service_unavailable_count))
                else:
                    backoff_time = min(self.conf_obj.BACKOFF_BASE ** attempt, self.conf_obj.BACKOFF_MAX)
                
                logger.debug(f"Backing off for {backoff_time}s before retry {attempt + 1}")
                time.sleep(backoff_time)
        
        logger.error(f"All {self.conf_obj.MAX_RETRIES_PER_REQUEST} attempts failed")
        return None
...
class PromptBuilder:
    ...
    def create_prompt(
        ...
        card_id: str,
        top_k: int,
        anchor_rule: Union[str, int],
        prompt_context: str,
        cluster_preferences: Dict[int, List[str]] = None, #TODO: ADDED FOR GEOM TIME OK
        poi_peak_hours: Dict[str, List[int]] = None,  #TODO: ADDED FOR GEOM TIME OK
    ) -> str:
        """
        ...
        seq_time = visits.apply(lambda row: f"{row['name_short']} ({row['timestamp']})",axis=1).tolist()
        ...
        if anchor_rule == "middle":
            ...
            history_time = seq_time[:anchor_idx]
        else:
            ...
            history_time = [p for i, p in enumerate(prefix_time) if i != anchor_idx]
        ...
        temporal_info = PromptBuilder.extract_temporal_features(df, card_id, anchor_idx) #TODO: ADDED FOR GEOM TIME
        ...
        if prompt_context == 'base':
            return f"""
                    Visited PoIs: {', '.join(history) if history else 'none'}
                    Current location: {current_poi}

                    Suggest the {top_k} most likely next PoIs considering typical tourist movement patterns in Verona.
                    Respond ONLY in JSON format: {{"prediction": ["PoI1", "PoI2", "PoI3", "PoI4", "PoI5"], "reason": "brief explanation"}}
                    """
        ...
        lat,lon = PromptBuilder.get_lat_long(current_poi, pois_df)

        if prompt_context == 'spatio':
            return f"""
                    Visited PoIs: {', '.join(history) if history else 'none'}
                    Current location: {current_poi} (GPS: {lat}, {lon})
                    Nearest POIs: {pois_list}

                    Suggest the {top_k} most likely next PoIs considering:
                    - Physical distance from current location
                    - Typical tourist route patterns in Verona
                    - Walking accessibility constraints (2km radius)
                        
                    Respond ONLY in JSON format: {{"prediction": ["PoI1", "PoI2", "PoI3", "PoI4", "PoI5"], "reason": "brief explanation"}}"""
        
        if prompt_context == 'spatio-temporal':
            return f"""
                Visit history with timestamps:{', '.join(history) if history_time else 'none'}
                Current location: {current_poi} (GPS: {lat}, {lon})
                Current time: {temporal_info['day']}, {temporal_info['minute']} {temporal_info['hour']}
                Nearest POIs: {pois_list}

                Suggest the {top_k} most likely next PoIs considering the current time({temporal_info['time_period']}), the temporal tourist patterns in Verona, suggest 5 most likely next PoIs.

                Respond ONLY in JSON format: {{"prediction": ["PoI1", "PoI2", "PoI3", "PoI4", "PoI5"], "reason": "brief explanation"}}"""
    
        cluster_top_pois = cluster_preferences.get(cluster_id, [])
        if prompt_context == 'spatio-temporal-popularity':
            cluster_ranking = " > ".join(cluster_top_pois[0]) if cluster_top_pois else "none" #top1

        if prompt_context == 'spatio-temporal-pref':
            cluster_ranking = " > ".join(cluster_top_pois[:5]) #all

        return f"""
                Cluster typical preference: {cluster_ranking}
                ...
                Suggest the {top_k} most likely next PoIs considering:
                - Cluster preferences and typical behavior
                - Current time ({temporal_info['time_period']}) and temporal patterns
                - Spatial proximity and walking constraints
                - Historical visit sequence. Suggest 5 most likely next PoIs that align with this tourist’s behavioral profile.
...
class ResultsManager:
    ...
    def add_result(self, result: Dict, batch_save_interval: int = 50):
        """Add a result to the buffer"""
        self._buffer.append(result)
        
        # Save if buffer is full
        if len(self._buffer) >= batch_save_interval:
            self.save_batch()
...
# ============= CARD PROCESSING ============= preso da verione base_parallel --> IN BASELINE TEST è DIVERSA
class CardProcessor:
    ...
    def __init__(
        ...
        cluster_preferences: Dict[int, List[str]], #TODO: ADDED FOR GEOM TIME ok
        pois_df: DataFrame,
        poi_peak_hours: Dict[str, List[int]],  #TODO: ADDED FOR GEOM TIME ok
        ...
        self.cluster_preferences = cluster_preferences  #TODO: ADDED FOR GEOM TIME OK
        self.pois_df = pois_df
        self.poi_peak_hours = poi_peak_hours #TODO: ADDED FOR GEOM TIME OK
        self.ollama_manager = ollama_manager
        self.checkpoint_manager = checkpoint_manager
        self.results_manager = results_manager
        self.conf_obj = ollama_manager.conf_obj
        ...
        try:
            ...
            try:
                prompt = PromptBuilder.create_prompt(
                    ...
                    card_id = card_id,
                    top_k=self.conf_obj.TOP_K,
                    anchor_rule=self.conf_obj.DEFAULT_ANCHOR_RULE,
                    prompt_context = self.conf_obj.CONTEXT,
                    cluster_preferences = self.cluster_preferences, #TODO: ADDED FOR GEOM TIME OK,
                    poi_peak_hours = self.poi_peak_hours,    #TODO: ADDED FOR GEOM TIME OK
                )
            ...
            if self.conf_obj.DEFAULT_ANCHOR_RULE == "middle":
                # For middle rule: anchor is middle of full sequence, target is next element
                anchor_idx = PromptBuilder.get_anchor_index(len(seq), self.conf_obj.DEFAULT_ANCHOR_RULE, is_full_sequence=True)
                current_poi = seq[anchor_idx]
                target = seq[anchor_idx + 1]  # Element immediately after anchor
                ...
                target = seq[-1]
                prefix = seq[:-1]
                anchor_idx = PromptBuilder.get_anchor_index(len(prefix), self.conf_obj.DEFAULT_ANCHOR_RULE)
            ...
            self.results_manager.add_result(result, self.conf_obj.BATCH_SAVE_INTERVAL)
...
class VisitFileProcessor:
    ...
    def __init__(self, ollama_manager: OllamaConnectionManager):
        ...
        self.ollama_manager = ollama_manager
        self.conf_obj = ollama_manager.conf_obj
        self.conf_obj.RESULTS_DIR.mkdir(exist_ok=True)
    ...
    def process_file(
        ...
        if not force and CheckpointManager.should_skip_file(
            visits_path, self.conf_obj.RESULTS_DIR, append
        ):
        ...
        # Initialize managers
        checkpoint_manager = CheckpointManager(visits_path, self.conf_obj.RESULTS_DIR)
        results_manager = ResultsManager(visits_path, self.conf_obj.RESULTS_DIR, append)
        ...
        try:
            ...
            card_processor = CardProcessor(
                filtered_df,
                user_clusters,
                cluster_preferences, # TODO: ADDED FOR GEOM TIME OK
                pois_df,
                poi_peak_hours, # TODO: ADDED FOR GEOM TIME OK
                self.ollama_manager,
                checkpoint_manager,
                results_manager
            )
    ...
    def _process_cards_parallel(
        ...
        optimal_workers = min(self.conf_obj.MAX_CONCURRENT_REQUESTS, len(cards_to_process))  # Use global limit instead of per-GPU
        ...
        for host in self.ollama_manager.hosts:
            try:
                test_resp = requests.post(
                    f"{host}/api/chat",
                    json={
                        "model": self.conf_obj.MODEL_NAME,
        ...
        with ThreadPoolExecutor(
            ...
            with tqdm(
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        result = future.result(timeout=self.conf_obj.REQUEST_TIMEOUT + 60)
                        
                        if result and result.get('status') == 'fatal_error':
                            logger.warning(f"Fatal error for card {card_id}")
                        
                        # Check circuit breaker
                        if stats.consecutive_failures >= self.conf_obj.CIRCUIT_BREAKER_THRESHOLD:
    ...
    def process_all_files(
        ...
        """
        poi_path = self.conf_obj.POI_FILE
        ...
        if single_file:
            ...
            for visit_file in sorted(visit_files):
                try:
                    if not force and CheckpointManager.should_skip_file(
                        visit_file, self.conf_obj.RESULTS_DIR, append
                    ):
                        skipped += 1
                        continue
    ...
    def _find_visit_files(self) -> List[Path]:
        ...
        for csv_path in self.conf_obj.DATA_DIR.rglob("*.csv"):
    ...
    def _resolve_file_path(self, file_path: str) -> Path:
        ...
        if not target.is_absolute():
            if not target.exists():
                ...
                target = self.conf_obj.DATA_DIR / file_path
                if not target.exists():
                    # Try just filename in data directory
                    target = self.conf_obj.DATA_DIR / Path(file_path).name
